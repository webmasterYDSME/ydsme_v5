...
function Form({ task }: { task: InboxTask }) {
  ...
  switch (task.type) {
    ...
    case "verification": return <form action={reviewPaidMembership} className="editor-form membership-verification-form"><input type="hidden" name="application_id" value={task.application.id}/><p className={styles.panelNote}>This person is already an active member. Record the routine check, or a rare denial.</p><label>Decision<select name="decision"><option value="approved">Confirm membership</option><option value="denied">Deny membership — arrange refund manually</option></select></label><label>Reason<textarea name="reason" minLength={5} maxLength={500} placeholder="Record what was checked and any supporting details." required/></label><PendingSubmitButton>Record verification</PendingSubmitButton></form>;
