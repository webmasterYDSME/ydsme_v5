...
Who runs membership is a database setting, not an environment variable, and an administrator changes it at Administrator > Membership system; it takes effect immediately and every change is recorded (there is nothing to redeploy). It has two values. `membermojo` (the starting value, and the value assumed if the setting cannot be read) means MemberMojo still runs membership: the website's apply, checkout and renew pages send people to MemberMojo, the membership area (`/admin/memberships`) is hidden, no card payments are taken and no membership emails are sent by the website. An administrator keeps the website register and sign-ins in step by uploading MemberMojo's member list at Administrator > Import MemberMojo list, then sending website invitations from the same page. `website` means the website runs membership itself, and a MemberMojo list import no longer archives anyone. Keep `membermojo` until the membership migrations are applied, the notification function is deployed, and reconciliation is approved. The switch page checks the Stripe keys, the email settings, the fees and the age of the last import first. To stop new applications, checkouts and reminders in an emergency, press Switch back to MemberMojo on the same page: one confirmation, effective at once. Retention (Setup > Old records) should stay switched off while in `membermojo` mode, because its warning emails are not sent then.

When migrations 0017 to 0025 are applied, also redeploy the two edge functions `run-membership-automation` and `deliver-membership-notifications`. Both now read the mode from the database (migration `202609210005_membership_mode_setting.sql`), so the old `MEMBERSHIP_MODE` secret is no longer used and can be removed from the deployed functions and from the hosting environment.

Website invitations for people added by the MemberMojo import are sent by the `member-invitations` scheduled job (migration 0023, every five minutes, through `run-membership-automation`), which an administrator starts once from Administrator > Member list > Website invitations. It sends 10 invitations per run (about 60 an hour), retries failures up to five times, waits 15 minutes when Supabase Auth reports that its hourly email limit was reached, and switches itself off when nobody is left. Invitations are created by Supabase Auth, not by the Resend queue, so before inviting everyone raise the hosted project's limit (Authentication > Rate limits > emails sent per hour; the default is 30 an hour with custom SMTP and 2 without). The job needs `SITE_URL` set as an edge function secret and the `project_url` and `maintenance_secret_key` vault secrets that the other scheduled jobs use. Copy `supabase/templates/invite.html` into the hosted project's Invite user email template (Authentication > Emails); the membership wording only appears if the hosted template is updated too. Configure separate Stripe test/live restricted keys, signed webhook secrets for the membership and donation endpoint registrations, and Customer Portal payment-method updates. Leave Stripe Tax disabled.
