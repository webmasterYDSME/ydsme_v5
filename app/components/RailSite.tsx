...
export function PageShell({children}:{children:React.ReactNode}) {
  ...
  return <>
    <header className="site-header">
      <Link href="/" className="brand"><span className="brand-roundel">Y</span><span>York Model<br/><b>Engineers</b></span></Link>
      <nav className={open?"main-nav open":"main-nav"}>{nav.map(([label,href])=><Link key={href} className={path===href?"active":""} href={href}>{label}</Link>)}<a className="login-mobile" href="https://www.yorkmodelengineers.co.uk/signin">Member login <ArrowUpRight size={15}/></a></nav>
