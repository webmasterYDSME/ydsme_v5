...
Keep `MEMBERSHIP_MODE=membermojo` until the membership migrations are applied, the notification function is deployed, and reconciliation is approved. Use `pilot` only in isolated local or preview environments with separate databases, `live` for public website membership, and `drain` to stop new applications, checkouts, reminders, and lapse automation while preserving officer recovery and signed webhook reconciliation. Configure separate Stripe test/live restricted keys, signed webhook secrets for the membership and donation endpoint registrations, and Customer Portal payment-method updates. Leave Stripe Tax disabled.
