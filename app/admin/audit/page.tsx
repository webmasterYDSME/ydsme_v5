...
import { AuditChangeDetails } from "@/app/components/AuditChangeDetails";
...
export default async function AuditPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  let query = createAdminClient().from("audit_logs").select("id,occurred_at,actor_id,actor_user_id,actor_role,action,entity_type,entity_id,summary", { count: "exact" });
  ...
  const actorIds = [...new Set((entries ?? []).flatMap((entry) => entry.actor_id ? [entry.actor_id] : []))];
  const actorResult = actorIds.length ? await createAdminClient().from("administrative_actors").select("id,display_name,reference_code,status").in("id", actorIds) : { data: [] };
