...
export async function createProject(formData: FormData) {
  ...
  const { data, error } = await client.from("member_projects").insert({
    ...parsed.data,
    owner_id: user.id,
...
export async function addProjectUpdate(formData: FormData) {
  ...
  const { error } = await client.rpc("create_member_project_update", {
    p_project_id: parsed.data.project_id,
    p_title: parsed.data.title,
    p_body: parsed.data.body,
    p_help_type: parsed.data.help_type,
    p_photo_paths: finalized,
  });
  if (error) {
    await removeProjectImages(finalized);
