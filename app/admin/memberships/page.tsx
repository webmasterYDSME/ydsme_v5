...
export default async function MembershipAdministration({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string; member?: string; section?: string }> }) {
  ...
  const sectionOpen = (section: string, openWhenBusy = false) => query.section
    ? query.section === section
    : openWhenBusy;
  const selectedTask = query.section ?? "applications";

  return <div className="portal-content membership-admin-page">
    <header className="portal-heading membership-admin-hero"><div><p className="eyebrow">Membership team</p><h1>Manage memberships</h1><p>Add and renew members, review applications, record payments and manage membership records.</p></div><div className="portal-filter-actions"><span className="count-badge"><UsersRound/>{members.length} members</span><form action={requestMembershipReportExport}><PendingSubmitButton className="button membership-export-button" pendingLabel="Preparing report…"><Download aria-hidden="true"/><span>Prepare records ZIP</span></PendingSubmitButton></form></div></header>
    ...
    {query.notice ? <p className="form-message success">{query.notice === "price-unchanged" ? "No fee change was needed. The current annual fee will continue automatically." : "Done. The membership record has been updated."}</p> : null}
    {reviewResult.data?.length ? <p className="form-message error">Website membership cannot be moved to live public use until {reviewResult.data.length} imported record{reviewResult.data.length === 1 ? "" : "s"} have been reviewed. Existing officer recovery remains available.</p> : null}

    <section className="membership-admin-summary">
      <article><UserCheck/><strong>{applications.filter((item) => item.status === "awaiting_approval").length + checkoutProblemCount}</strong><span>Applications needing action</span></article>
      <article><Banknote/><strong>{applications.filter((item) => item.status.startsWith("awaiting_") && item.status !== "awaiting_approval").length + (pendingOfflineResult.data?.length ?? 0)}</strong><span>Payments to record</span></article>
      <article><Award/><strong>{honoraryResult.data?.length ?? 0}</strong><span>Honorary members</span></article>
      <article><BellRing/><strong>{deliveryProblemCount}</strong><span>Emails needing attention</span></article>
    </section>

    <nav className="membership-admin-jump" aria-label="Membership page sections">
      <span>Common tasks</span>
      <Link href="/admin/memberships?section=applications#applications" aria-current={selectedTask === "applications" ? "page" : undefined}>Applications{applications.length ? <span className="membership-nav-count">{applications.length}</span> : null}</Link>
      <Link href="/admin/memberships?section=online-payment-problems#online-payment-problems" aria-current={selectedTask === "online-payment-problems" ? "page" : undefined}>Online payments{checkoutProblemCount ? <span className="membership-nav-count">{checkoutProblemCount}</span> : null}</Link>
      {(pendingOfflineResult.data?.length ?? 0) > 0 ? <Link href="/admin/memberships?section=pending-payments#pending-payments" aria-current={selectedTask === "pending-payments" ? "page" : undefined}>Waiting for payment<span className="membership-nav-count">{pendingOfflineResult.data?.length}</span></Link> : null}
      ...
      <Link href="/admin/memberships?section=honorary#honorary" aria-current={selectedTask === "honorary" ? "page" : undefined}>Honorary membership</Link>
      <Link href="/admin/memberships?section=plans#plans" aria-current={selectedTask === "plans" ? "page" : undefined}>Types and prices</Link>
    </nav>

    <MembershipAdminSection id="applications" className="membership-admin-primary membership-admin-predominant" eyebrow="Main work area" title="Applications and payments to check" description="Approve eligible applications and record payments made by cash, bank transfer or cheque." icon={<CreditCard/>} defaultOpen={sectionOpen("applications", true)}><div className="membership-queue-list">{applications.map((application) => {
    ...
    <MembershipAdminSection id="online-payment-problems" className={checkoutProblemCount ? "membership-attention-section membership-admin-predominant" : "membership-admin-predominant"} eyebrow={checkoutProblemCount ? "Needs attention" : "System check"} title={checkoutProblemCount ? "Online payments need attention" : "Online payments are ready"} description={checkoutProblemCount ? "Payment pages, verified payment messages and automatic-renewal changes that need an officer appear here." : "No unresolved payment-page, payment-message or automatic-renewal problems were found."} icon={checkoutProblemCount ? <CreditCard/> : <ShieldCheck/>} defaultOpen={sectionOpen("online-payment-problems", checkoutProblemCount > 0)}><div className="membership-queue-list">
      {!paymentConfigurationReady ? <article><div><strong>Online payment configuration is incomplete</strong><p>A restricted payment key and signed membership webhook secret are required before launch. No success page can activate membership.</p></div></article> : null}
      ...
      {(checkoutAttemptResult.data ?? []).map((problem) => <article key={problem.id}><div><strong>{problem.status === "payment_review" ? "Possible duplicate payment" : "Payment page failed"}</strong><p>{problem.last_error || "The payment attempt could not be completed and needs checking."}</p><small>Last updated {new Date(problem.updated_at).toLocaleString("en-GB")}</small></div></article>)}
      {(webhookFailureResult.data ?? []).map((problem) => <article key={problem.stripe_event_id}><div><strong>Verified payment message needs retrying</strong><p>The payment service message was saved safely but could not be applied. No success page has granted membership.</p><small>{problem.event_type} · {problem.last_error || "Retry required"}</small></div></article>)}
      {(providerCommandResult.data ?? []).map((problem) => <article key={problem.id}><div><strong>Automatic-renewal update needs retrying</strong><p>{problem.command_type.replaceAll("_", " ")} · {problem.attempts} attempt{problem.attempts === 1 ? "" : "s"}</p><small>{problem.last_error || "The payment service update is waiting to be retried."}</small></div></article>)}
    ...
    {(pendingOfflineResult.data ?? []).length ? <MembershipAdminSection id="pending-payments" eyebrow="Payment required" title="Members waiting for payment" description="Record the payment after the full amount has arrived." icon={<Banknote/>} defaultOpen={sectionOpen("pending-payments")}><div className="membership-queue-list">{pendingOfflineResult.data?.map((term) => <article key={term.id}><div><strong>{memberMap.get(term.member_id)?.full_name || "Member"}</strong><span>{term.membership_year} membership · {paymentMethodName(term.expected_payment_method)} · {money(term.amount_due_pence)}</span><small>Membership starts after full payment is confirmed.</small></div><form action={confirmExistingMemberOfflineRenewal} className="stack-form"><input type="hidden" name="member_id" value={term.member_id}/><input type="hidden" name="membership_year" value={term.membership_year}/><input type="hidden" name="payment_method" value={term.expected_payment_method || "cash"}/><label>Receipt or payment reference<input name="payment_reference" required minLength={2} maxLength={120}/></label><label>Date received<input type="date" name="received_on" defaultValue={today} required/></label>{term.expected_payment_method === "cheque" ? <label className="checkbox-row"><input type="checkbox" name="cleared" required/>Cheque cleared</label> : null}<PendingSubmitButton pendingLabel="Activating…">Mark paid and activate</PendingSubmitButton></form></article>)}</div></MembershipAdminSection> : null}
    ...
    {selectedMemberId ? <MembershipAdminSection id="member-history" className="membership-member-history" eyebrow="Member history" title={memberMap.get(selectedMemberId)?.full_name || "Member"} description="Review contact ownership, portal access, eligibility, memberships and payments." icon={<CreditCard/>} defaultOpen={sectionOpen("member-history", true)}>
      <div className="membership-admin-grid">
        ...
        <div className="editor-form"><h3>Personal portal login</h3><p className="form-help">A shared correspondence address never grants shared portal access. Each portal-enabled member needs a unique login.</p>{memberMap.get(selectedMemberId)?.auth_user_id ? <form action={removeMemberPortalLogin} className="stack-form"><input type="hidden" name="member_id" value={selectedMemberId}/><label>Reason for removing access<textarea name="reason" minLength={5} maxLength={500} required/></label><PendingSubmitButton className="danger-button" pendingLabel="Removing…">Remove personal portal access</PendingSubmitButton></form> : <form action={assignMemberPortalLogin} className="stack-form"><input type="hidden" name="member_id" value={selectedMemberId}/><label>Unique login email<input type="email" name="login_email" required/></label><label>Reason for assigning this login<textarea name="reason" minLength={5} maxLength={500} required/></label><PendingSubmitButton pendingLabel="Assigning…">Assign or invite portal login</PendingSubmitButton></form>}</div>
      </div>
