...
export type UploadIntentResult =
  | { ok: true; bucket: "images" | "documents" | "project-images"; path: string; token: string }
...
const rules = {
  ...
  "project-image": { capability: null, bucket: "project-images" as const, maximum: 8 * 1024 * 1024, types: ["image/jpeg", "image/png", "image/webp", "image/avif"] },
};

export async function createUploadIntent(input: unknown): Promise<UploadIntentResult> {
  const parsed = z.object({ kind: z.enum(["event-image", "committee-image", "document", "project-image"]), name: z.string().min(1).max(255), size: z.number().int().positive(), type: z.string().max(100) }).safeParse(input);
  if (!parsed.success) return { ok: false, error: "Invalid upload request." };
  const rule = rules[parsed.data.kind];
  const { user } = rule.capability ? await requireCapability(rule.capability) : await requireUser();
