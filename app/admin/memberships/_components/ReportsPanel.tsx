...
import { dateTimeLabel } from "@/lib/membership-admin/format";
...
export async function ReportsPanel() {
  ...
  return <section className={styles.card}>
    ...
    <div className="membership-queue-list">{reportExports.map((report) => <article key={report.id}><div><strong>{report.status === "ready" ? "Complete membership records" : report.status === "failed" ? "Report needs retrying" : "Report is being prepared"}</strong><span>Requested {dateTimeLabel(report.created_at)}</span><small>{report.status === "ready" ? `Ready until ${dateTimeLabel(report.expires_at)}` : report.last_error || "Large registers may take a few minutes."}</small></div>{report.status === "ready" ? <Link className="button outline" href={`/admin/memberships/export?id=${report.id}`}>Download records</Link> : null}</article>)}{!reportExports.length ? <div className="membership-empty-state"><Download/><strong>No reports prepared yet</strong><p>Use “Prepare records download” above when bookkeeping or legal records are needed.</p></div> : null}</div>
  </section>;
}
