...
When migrations 0017 to 0021 are applied, also redeploy the two edge functions `run-membership-automation` and `deliver-membership-notifications`. Configure separate Stripe test/live restricted keys, signed webhook secrets for the membership and donation endpoint registrations, and Customer Portal payment-method updates. Leave Stripe Tax disabled.
