...
import { membershipRenewalIsOpen, membershipRenewalYear } from "@/lib/membership-rules";
...
export default async function Account({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string }> }) {
  ...
  const renewalAvailable = membershipRenewalIsOpen(new Date()) || membership?.member.effective_state === "lapsed";
