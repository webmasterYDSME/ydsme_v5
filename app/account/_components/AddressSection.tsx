...
export function AddressSection({ details, message, today }: { details: OwnMemberDetails | null; message: SectionMessage | null; today: string }) {
  return <section className={styles.card} id="address" aria-labelledby="address-heading">
    <header className={styles.cardHead}>
      <span className={styles.badge}><MapPin/></span>
      <div className={styles.headText}><h2 id="address-heading">Address and date of birth</h2><p>Where we can post things to you, and your date of birth so we apply the right membership fee.</p></div>
    </header>
...
function Form({ details, today }: { details: Extract<OwnMemberDetails, { linked: true }>; today: string }) {
  return <form action={updateMemberDetails} className={styles.formGrid}>
    ...
    {details.birthDateLocked && details.dateOfBirth ? <div className={`${styles.field} ${styles.wide}`}>
    ...
    </div> : <div className={`${styles.field} ${styles.wide}`}>
    ...
    <div className={styles.wide}><PendingSubmitButton className="button dark" pendingLabel="Saving…">Save address</PendingSubmitButton></div>
  </form>;
}
