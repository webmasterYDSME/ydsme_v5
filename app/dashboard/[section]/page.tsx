...
export default async function DocumentsPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="portal-content"><header className="portal-heading"><div><p className="eyebrow dark">Document archive</p><h1>{config.title}</h1><p>{config.copy}</p></div></header>
    ...
    {editable ? <details className="manager-panel"><summary><Upload/>Upload a PDF</summary><form action={uploadDocument} className="stack-form upload-form"><label>Document name<input name="name" required/></label><label>Category<select name="category" defaultValue={config.categories[0]}>{config.categories.map(category => <option key={category} value={category}>{category.replaceAll("-", " ")}</option>)}</select></label><label>Description<textarea name="descriptions" rows={3}/></label><SignedUploadField kind="document" label="PDF file" required/><button className="button dark" type="submit"><Upload/>Upload document</button></form></details> : null}
    <section className="document-grid">{docs.map(doc => <article key={doc.id}><FileText/><div><span>{doc.category.replaceAll("-", " ")} · version {doc.version} · {doc.lifecycle_status}</span><h2>{doc.name}</h2><p>{doc.descriptions}</p><time>{format(new Date(doc.created_at), "d MMMM yyyy")}</time></div><div className="row-actions">{doc.signedUrl ? <a href={doc.signedUrl} target="_blank" rel="noreferrer"><Download/>Open PDF</a> : null}{editable ? doc.lifecycle_status === "archived" ? <><form action={restoreDocument}><input type="hidden" name="id" value={doc.id}/><button type="submit"><RotateCcw/>Restore</button></form>{isAdministrator(role) ? <details><summary>Permanent purge</summary><form action={purgeDocument} className="stack-form"><input type="hidden" name="id" value={doc.id}/><label>Type PURGE DOCUMENT<input name="confirmation" required/></label><button className="danger-button" type="submit">Purge file</button></form></details> : null}</> : <><details><summary><Pencil/>Edit details</summary><form action={updateDocumentMetadata} className="stack-form"><input type="hidden" name="id" value={doc.id}/><label>Name<input name="name" defaultValue={doc.name} required/></label><label>Description<textarea name="descriptions" defaultValue={doc.descriptions}/></label><button type="submit">Save metadata</button></form></details><details><summary><Upload/>Replace version</summary><form action={replaceDocumentVersion} className="stack-form"><input type="hidden" name="id" value={doc.id}/><SignedUploadField kind="document" label="Replacement PDF" required/><button type="submit">Replace PDF</button></form></details><form action={deleteDocument}><input type="hidden" name="id" value={doc.id}/><button type="submit"><Archive/>Archive</button></form></> : null}</div></article>)}{!docs.length ? <div className="empty-document"><FileText/><h2>No documents yet</h2><p>The archive is ready for its first file.</p></div> : null}</section>
  </div>;
}
