...
`MEMBERSHIP_MODE` has two values. `membermojo` (the default) means MemberMojo still runs membership: the website's apply, checkout and renew pages send people to MemberMojo, the membership area (`/admin/memberships`) is hidden, no card payments are taken and no membership emails are sent by the website. An administrator keeps the website register and sign-ins in step by uploading MemberMojo's member list at Administrator > Import MemberMojo list, then sending website invitations from the same page. `website` means the website runs membership itself; the older values `pilot`, `live` and `drain` are read as `website`. Keep `membermojo` until the membership migrations are applied, the notification function is deployed, and reconciliation is approved. To stop new applications, checkouts and reminders in an emergency, set `membermojo` and redeploy. Retention (Setup > Old records) should stay switched off while in `membermojo` mode, because its warning emails are not sent then.

When migrations 0017 to 0023 are applied, also redeploy the two edge functions `run-membership-automation` and `deliver-membership-notifications`.
