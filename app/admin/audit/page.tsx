...
export default async function AuditPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  let query = createAdminClient().from("audit_logs").select("id,occurred_at,actor_user_id,actor_role,action,entity_type,entity_id,summary", { count: "exact" });
