...
export default async function MembershipAdministration({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string; member?: string; section?: string; view?: string; q?: string; page?: string }> }) {
  ...
  const attentionCount = (pendingOfflineResult.data?.length ?? 0) + applications.length + (studentRequestResult.data?.length ?? 0) + manualContactTasks.length + deliveryProblemCount + checkoutProblemCount + (paymentReviewResult.data?.length ?? 0) + (honoraryConflictResult.data?.length ?? 0);
  ...
  return <div className={`portal-content membership-admin-page ${styles.workspace}`}>
    <header className="portal-heading"><div><p className="eyebrow dark">Membership team</p><h1>Manage memberships</h1></div><Link className="button dark" href="/admin/memberships?section=add-member"><UserPlus/>Add membership</Link></header>
    ...
    <PortalTabs label="Membership workspace" tabs={[
      { href: "/admin/memberships", label: "Needs attention", count: attentionCount, current: view === "attention" },
      { href: "/admin/memberships?view=members", label: "Member register", current: view === "members" },
      { href: "/admin/memberships?view=payments", label: "Payments", current: view === "payments" },
    ...
    {view === "attention" && <>
      <div className={styles.overview}>
        <Link href="/admin/memberships?section=applications"><UserCheck/><strong>{applications.length}</strong><span>Applications to review</span></Link>
        <Link href="/admin/memberships?view=payments"><Banknote/><strong>{pendingOfflineResult.data?.length ?? 0}</strong><span>Renewal payments waiting</span></Link>
