...
export function BulkInviteForm() {
  ...
  return <form action={bulkInviteMembers} className="stack-form"><label>CSV file<input type="file" name="file" accept="text/csv,.csv" required onChange={event => void inspect(event.target.files?.[0])}/></label><p className="form-help">Maximum 250 rows and 1 MB. Existing addresses will be reported rather than overwritten.</p>{preview.length ? <div className="import-preview"><h2>Import preview</h2><p>Showing the first {preview.length} data rows. Nothing is sent until you confirm below.</p>{preview.map((row, index) => <div key={index}><strong>{row.name}</strong><span>{row.email}</span></div>)}</div> : null}<PendingSubmitButton className="button dark" pendingLabel="Sending invitations…"><FileUp/>Send invitations</PendingSubmitButton></form>;
}
