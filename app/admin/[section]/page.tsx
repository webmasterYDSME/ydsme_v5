...
export default async function AdminSection({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<Query> }) {
  ...
  return <div className="portal-content"><header className="portal-heading"><div><p className="eyebrow dark">Administrator only</p><h1>Members</h1><p>Invite members, manage website access and assign Society roles from one register.</p></div><span className="count-badge"><UsersRound/>{count ?? 0} {status}</span></header>{query.error ? <p className="form-message error">{query.error}</p> : null}{notice ? <p className="form-message success">{notice}</p> : null}
