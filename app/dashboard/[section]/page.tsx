...
import { deleteDocument, purgeDocument, restoreDocument } from "@/lib/actions/content";
import { DocumentUploadDialog } from "@/app/components/DocumentUploadDialog";
import { DocumentEditorDialog } from "@/app/components/DocumentEditorDialog";
...
export default async function DocumentsPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ error?: string; status?: string; page?: string }> }) {
  ...
  return <div className="portal-content"><header className="portal-heading"><div><p className="eyebrow dark">Document archive</p><h1>{config.title}</h1><p>{config.copy}</p></div>{editable ? <DocumentUploadDialog categories={config.categories}/> : null}</header>
