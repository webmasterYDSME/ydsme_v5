...
export default async function AdminBookingsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <div className="portal-content"><header className="portal-heading"><div><p className="eyebrow dark">Visitor management</p><h1>Visitor bookings</h1><p>Search, check in, contact and manage visitor groups.</p></div><div className="booking-heading-summary"><span className="count-badge"><UsersRound/>{confirmedPeople} active {confirmedPeople === 1 ? "visitor" : "visitors"}{selectedEvent?.booking_capacity ? ` / ${selectedEvent.booking_capacity}` : ""}</span><small>{bookingCount} matching {bookingCount === 1 ? "booking" : "bookings"}</small></div></header>
    ...
    <form className="booking-filter-panel" action="/admin/bookings" method="get">
      <div className="booking-filter-heading"><div><h2>Find a booking</h2><p>Search the visitor list or narrow it by event and status.</p></div><Link prefetch={false} className="button secondary" href={`/admin/bookings/export?${exportParams}`}><Download/>Download spreadsheet</Link></div>
