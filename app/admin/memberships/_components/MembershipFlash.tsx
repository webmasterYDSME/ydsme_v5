...
export function MembershipFlash({ query }: { query: { error?: Value; notice?: Value } }) {
  ...
  return <>
    {error ? <p className="form-message error" role="alert">That change could not be completed. Check the details and try again. <small>({error.replaceAll("-", " ")})</small></p> : null}
