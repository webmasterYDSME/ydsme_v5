...
export default async function Events() {
  ...
  const eventJsonLd = {
    ...
    "@graph": events.map((event) => ({
      ...
      isAccessibleForFree: !event.is_ticket_required,
      ...
      ...(event.reservation_link ? { url: event.reservation_link } : { url: `${SITE_URL}/events` }),
    })),
  };
  ...
  return <PageShell><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJsonLd(eventJsonLd) }} /><InnerHero kicker="The public running board" title={<>What’s next<br/><em>down the line.</em></>} copy="Public open days and special running days—live dates from the Society timetable, ready for your visit." image="/images/engine.webp" imageAlt="A miniature steam locomotive prepared for a public running day"/>
    ...
    {more.length ? <section className="events-list"><div className="events-label"><span>MORE PUBLIC DEPARTURES</span><span>YORK · {new Date().getFullYear()}</span></div>{more.map((event, index)=><Reveal key={event.id} delay={index*.06}><article className="event-row"><div className="event-date"><strong>{format(parseISO(event.start_date), "dd")}</strong><span>{format(parseISO(event.start_date), "MMM").toUpperCase()}</span></div><div><p className="eyebrow dark">Public running day</p><h3>{event.name}</h3><p>{event.descriptions}</p></div><time dateTime={`${event.start_date}T${eventTime(event.start_time)}`}>{eventTime(event.start_time)}</time>{event.is_ticket_required && event.reservation_link ? <a href={event.reservation_link} aria-label={`Book ${event.name}`} target="_blank" rel="noreferrer"><ArrowRight/></a> : <ArrowRight aria-hidden="true"/>}</article></Reveal>)}</section> : null}
