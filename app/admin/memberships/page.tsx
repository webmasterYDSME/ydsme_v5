...
export default async function MembershipAdministration({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string; member?: string; section?: string }> }) {
  ...
  return <div className="portal-content membership-admin-page">
    <header className="portal-heading membership-admin-hero"><div><p className="eyebrow">Membership team</p><h1>Manage memberships</h1><p>Add and renew members, review applications, record payments and manage membership records.</p></div><div className="portal-filter-actions"><span className="count-badge"><UsersRound/>{members.length} {members.length === 1 ? "member" : "members"}</span><form action={requestMembershipReportExport}><PendingSubmitButton className="button membership-export-button" pendingLabel="Preparing report…"><Download aria-hidden="true"/><span>Prepare records download</span></PendingSubmitButton></form></div></header>
