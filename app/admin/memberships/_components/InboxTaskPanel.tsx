...
/** The fee as pounds and pence for an input box, or empty when it is not known. */
const poundsValue = (pence: number | null | undefined) => (pence == null ? "" : (pence / 100).toFixed(2));
...
function Form({ task }: { task: InboxTask }) {
  ...
  switch (task.type) {
    case "application-payment": {
      const { application, received, amountDuePence } = task;
