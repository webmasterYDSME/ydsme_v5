...
export default async function DocumentsPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ error?: string; status?: string; page?: string }> }) {
  ...
  return <div className="portal-content"><header className="portal-heading"><div><p className="eyebrow dark">Document archive</p><h1>{config.title}</h1><p>{config.copy}</p></div></header>
    ...
    {editable ? <nav className="status-filter" aria-label={`Filter ${config.title.toLowerCase()} by status`}><Link href={`/dashboard/${section}?status=published`} aria-current={status === "published" ? "page" : undefined}>Published <span>{publishedCount}</span></Link><Link href={`/dashboard/${section}?status=archived`} aria-current={status === "archived" ? "page" : undefined}>Archived <span>{archivedCount}</span></Link></nav> : null}
