-- Newsletter: people who have not unsubscribed from MemberMojo's group emails are subscribed when they are
-- added, and the consent record says where it came from. Existing members, Juniors and people without an
-- email address are never subscribed by the import.
...
alter table public.members add constraint members_newsletter_consent_source_check
  check (newsletter_consent_source is null or newsletter_consent_source in ('paper_form','in_person','phone','membermojo_list'));
