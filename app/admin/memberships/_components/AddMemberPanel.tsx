...
export async function AddMemberPanel({ mode, closeHref }: { mode: "member" | "honorary"; closeHref: string }) {
  ...
  return <SidePanel wide key={mode} closeHref={closeHref} label="Add a member" eyebrow="Offline application" eyebrowClassName={styles.pillMute} title={mode === "member" ? "Add a membership" : "Add an honorary member"}>
    ...
    {mode === "member" ? <>
      <p className={styles.panelNote}>For someone who applied in person or cannot use the online application. Their membership type and amount due are calculated from their date of birth and start date.</p>
      ...
      <p className={styles.panelNote}>Payment-free lifetime membership. Every change requires a reason and is kept in the member’s history.</p>
      <form action={createHonoraryMember} className="editor-form"><label>Full name<input name="full_name" required/></label><label>Email address <em>Optional</em><input name="contact_email" type="email"/></label><label>Telephone number <em>Optional</em><input name="contact_number"/></label><label>Start date<input name="effective_from" type="date" min={today} defaultValue={nextYearStart} required/></label><label>Reason for honorary membership<textarea name="reason" minLength={5} maxLength={500} required/></label><PendingSubmitButton className="button dark" pendingLabel="Adding…">Add honorary member</PendingSubmitButton></form>
    </>}
  </SidePanel>;
}
