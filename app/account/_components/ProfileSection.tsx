...
/** Name, title and phone number. Saving also updates the linked membership record. */
export function ProfileSection({ title, fullName, contactNumber }: { title: string; fullName: string; contactNumber: string }) {
  return <Section id="profile" eyebrow="Profile" title="Your details" icon={UserRound} description="How the Society knows you. Changes here also update your membership record.">
    <form action={updateProfile} className={styles.stackForm}>
      <div className={styles.formGrid}>
        ...
        <label className={`${styles.field} ${styles.wide}`}>Phone number<input className={styles.input} name="contact_number" type="tel" defaultValue={contactNumber} autoComplete="tel"/></label>
      </div>
